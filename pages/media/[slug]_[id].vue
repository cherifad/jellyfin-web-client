  ...
  <div v-if="itemDetails" class="px-7">
    <div class="flex justify-between">
      <div class="flex gap-4 w-3/5">
        <img :src="posterUrl" class="rounded-lg block mx-auto w-1/3" alt="dd" />
      ...
      <div class="w-2/5 flex justify-end">
        <div class="h-fit overflow-hidden p-5 w-3/5">
          <VideoPlayer
            v-if="itemDetails.RemoteTrailers.length > 0"
            :videoUrl="itemDetails.RemoteTrailers[0].Url"
            :videoThumbnail="null"
          />
        </div>
      </div>
    ...
    <div
      class="p-2 my-10 rounded-lg bg-pink-500 sticky bottom-3 z-30 flex items-center gap-3 flex-wrap"
    >
      <div
        class="bg-white flex items-center gap-2 text-black rounded-lg p-4 group cursor-pointer"
      >
        <PlayIcon class="h-8 w-8 group-hover:scale-125 transition-transform" />
        <span class="group-hover:scale-110 transition-transform">{{
          itemDetails.UserData.PlaybackPositionTicks
            ? `Resume
          at ${
            ticksToHoursAndMinutes(itemDetails.UserData.PlaybackPositionTicks)
              .minutes
          }`
            : "Play"
        }}</span>
      ...
      <p
        ...
        v-if="lang.subtitleLanguages.length > 0"
      >
...
async function getDetails() {
  const res = await getItemDetails(id.value);
  ...
  lang.value = getAudioSubtitleLanguages(
    itemDetails.value.MediaSources[0].MediaStreams
  );
