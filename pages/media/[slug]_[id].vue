<template>
  <div v-if="itemDetails" class="px-7">
    <div class="flex justify-between">
      <div class="flex gap-4 w-3/5">
        ...
        <MediaDetailsItemDetails :overview="itemDetails.Overview" :genres="itemDetails.Genres" :name="itemDetails.Name"
          ...
          <vue-plyr v-if="itemDetails.RemoteTrailers.length > 0">
            <div class="plyr__video-embed w-3/5 h-fit">
              <iframe :src="
                              ...
                              " allowfullscreen allowtransparency allow="autoplay" class="w-full h-3/5"></iframe>
            </div>
          </vue-plyr>
        </div>
      </div>
    ...
    <div class="mt-10">
      <div class="mb-5 flex justify-between items-center">
        ...
        <div class="flex">
          <ChevronLeftIcon class="h-6 w-6 cursor-pointer" @click="changeSlide(false)" />
          <ChevronRightIcon class="h-6 w-6 cursor-pointer" @click="changeSlide(true)" />
        </div>
      </div>
      <swiper :spaceBetween="30" :modules="[SwiperAutoplay]" :slidesPerView="6"
        :autoplay="{ delay: 2000, pauseOnMouseEnter: true }" ref="swiperRef">
        <swiper-slide v-for="cast in itemDetails.People" :key="cast.Id">
          <SinglePeople :key="cast.Id" :peopleName="cast.Name" :peopleRole="cast.Role"
            :peopleImageTag="cast.PrimaryImageTag" :peopleId="cast.Id" />
        </swiper-slide>
      </swiper>
    </div>
    <!-- recommendations -->
    <div class="mt-10">
      ...
      <swiper :spaceBetween="30" :modules="[SwiperAutoplay]" :slidesPerView="4"
        :autoplay="{ delay: 2000, pauseOnMouseEnter: true }" ref="swiperRef">
        <swiper-slide v-for="recommendation in recommendations" :key="recommendation.Id">
          <SectionsSingleItem :key="recommendation.Id" :item-title="recommendation.Name"
            :item-img="recommendation.BackdropImageTags[0]" :item-id="recommendation.Id"
    ...
    <div class="p-2 my-10 rounded-lg bg-pink-500 sticky bottom-3 z-30 flex items-center gap-3 flex-wrap">
      <div class="bg-white flex items-center gap-2 text-black rounded-lg p-4 group cursor-pointer">
        ...
        <span class="group-hover:scale-110 transition-transform">{{ itemDetails.UserData.PlaybackPositionTicks ? `Resume
          at ${ticksToHoursAndMinutes(itemDetails.UserData.PlaybackPositionTicks).minutes}` : 'Play'}}</span>
      </div>
  ...
  <div v-else class="h-screen flex flex-1 justify-center items-center">
    <div class="flex gap-3 text-black bg-white px-4 py-2 font-semibold w-fit rounded-md items-center">
...
async function getDetails() {
  ...
  posterUrl.value = `${config.selectedServerValue.url}/Items/${itemDetails.value.Id}/Images/Primary?maxHeight=500&tag=${itemDetails.value.ImageTags.Primary}`;
  backdropUrl.value = `${config.selectedServerValue.url}/api/v1/GetImage/${runtimeConfig.apiKey}/${itemDetails.value.ImageTags.Backdrop}?width=300`;
